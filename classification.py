...
def cmatrix_batch(model, val_loader, normalize=False, device="cuda"):
    """Calculate the confusion matrix given a dataloader. This is
    useful when the data is in batches, or is shuffled
    """
    num_classes = len(val_loader.dataset.classes)
    cmatrix = torch.zeros(num_classes, num_classes, dtype=int)
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(val_loader):
            ...
            for t, p in zip(labels, preds):
                cmatrix[t, p] += 1
    cmatrix = cmatrix.numpy()
    if normalize:
        cmatrix = cmatrix / cmatrix.sum(axis=1, keepdims=True)
    return cmatrix


def get_preds(model, val_loader, device="cuda"):
    """Get predictions from a model. It's recommended to not shuffle
    the data.
    """
    pred_list = []
    ...
    return torch.cat(pred_list).cpu().numpy()
